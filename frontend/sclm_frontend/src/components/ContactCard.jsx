...
const ContactCard = ({ contact, onEdit, onDelete }) => {
  return (
    <div className="bg-gray-400 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
      {/* Header with gradient */}
      <div className="bg-gradient-to-r from-blue-800 to-gray-600 h-24 relative">
        <div className="absolute -bottom-12 left-1/2 transform -translate-x-1/2">
          <div className="w-24 h-24 rounded-full bg-gray-400 border-4 border-blue-800 shadow-lg flex items-center justify-center">
            ...
            <div className="space-y-2">
              {contact.emailAddresses.map((email, index) => (
                <div key={index} className="flex items-start gap-2 text-sm">
                  <Mail size={16} className="text-blue-700 mt-0.5 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-800 truncate">{email.email}</p>
                    <span className="text-xs text-gray-700 uppercase">{email.type}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {contact.phoneNumbers && contact.phoneNumbers.length > 0 && (
            <div className="space-y-2">
              {contact.phoneNumbers.map((phone, index) => (
                <div key={index} className="flex items-start gap-2 text-sm">
                  <Phone size={16} className="text-blue-700 mt-0.5 flex-shrink-0" />
                  <div className="flex-1">
                    <p className="text-gray-800">{phone.number}</p>
                    <span className="text-xs text-gray-700 uppercase">{phone.type}</span>
                  </div>
                </div>
              ))}
            </div>
